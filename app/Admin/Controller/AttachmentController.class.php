...
class AttachmentController extends AdminCoreController {
    protected function _upload_init($upload) {
        ...
        $allow_max && $upload->maxSize = $allow_max * 5120;   //文件大小限制
        $allow_exts && $upload->allowExts = $allow_exts;  //文件类型限制
        $upload->savePath =  './data/attachment/editer/' . $file_type . '/';
    ...
    public function editer_upload() {
        ...
        if ($result['error']) {
            ...
            echo json_encode(array('error'=>0, 'url'=>__ROOT__.'/data/attachment/editer/' . $file_type . '/' . $result['info'][0]['savename']));
        }
        exit;
    }
    ...
    public function editer_manager() {
        $root_path = './data/attachment/editer/';
        $root_url = '/data/attachment/editer/';
