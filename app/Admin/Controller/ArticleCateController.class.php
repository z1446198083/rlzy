...
class ArticleCateController extends AdminCoreController {
    ...
    public function index() {
        ...
        $array = array();
        foreach($result as $r) {
            $r['str_img'] = $r['img'] ? '<div class="img_border"><img src="'.attach($r['img'], 'ArticleCate').'" width="26" height="26" class="J_preview" data-bimg="'.attach($r['img'], 'ArticleCate').'"/></div>' : '';
//            $r['str_imgs'] = $r['imgs'] ? '<div class="img_border"><img src="'.attach($r['imgs'], 'ArticleCate').'" width="26" height="26" class="J_preview" data-bimg="'.attach($r['imgs'], 'ArticleCate').'"/></div>' : '';
            ...
            $r['str_manage'] = '<a href="javascript:;" class="J_showdialog" data-uri="'.U('ArticleCate/add',array('pid'=>$r['id'])).'" data-title="'.L('add_article_cate').'" data-id="add" data-width="500" data-height="360">'.L('add_article_subcate').'</a> |
                                <a href="javascript:;" class="J_showdialog" data-uri="'.U('ArticleCate/edit',array('id'=>$r['id'])).'" data-title="'.L('edit').' - '. $r['name'] .'" data-id="edit" data-width="500" data-height="360">'.L('edit').'</a> |
    ...
    public function ajax_upload_img() {
        ...
        if (!empty($_FILES['img']['name'])) {
            $result = $this->_upload($_FILES['img'], 'ArticleCate', array('width'=>C('pin_article_cate_img.width'), 'height'=>C('pin_article_cate_img.height')));
            if ($result['error']) {
    ...
    public function ajax_upload_imgs() {
        ...
        if (!empty($_FILES['imgs']['name'])) {
            $result = $this->_upload($_FILES['imgs'], 'ArticleCate', array('width'=>C('pin_article_cate_img.width'), 'height'=>C('pin_article_cate_img.height')));
            if ($result['error']) {
                $this->ajax_return(0, $result['info']);
            } else {
                $ext = array_pop(explode('.', $result['info'][0]['savename']));
